...
public class FinishActivity extends BaseSetupWizardActivity {
    ...
    private void animateLetters() {
        ...
        final Handler letterHandler = new Handler();
        for (int i = 0; i < text.length(); i++) {
            ...
            letterHandler.postDelayed(() -> anim.start(), 800 + (index * 80));
        }

        letterHandler.postDelayed(() -> {
            TextView readyText = findViewById(R.id.ready_text);
            if (readyText == null) return;
            readyText.setTranslationY(30f);
            readyText.setAlpha(0f);
            readyText.setVisibility(View.VISIBLE);

            ObjectAnimator slide = ObjectAnimator.ofFloat(readyText, View.TRANSLATION_Y, 30f, 0f);
            ObjectAnimator fade = ObjectAnimator.ofFloat(readyText, View.ALPHA, 0f, 1f);
            AnimatorSet set = new AnimatorSet();
            set.setDuration(400);
            set.setInterpolator(new AccelerateDecelerateInterpolator());
            set.playTogether(slide, fade);
            set.start();
        }, 800 + text.length() * 80 + 100);
    }
